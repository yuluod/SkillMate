...
- 支持从统一库按平台启用到所有项目或指定项目，停用后保留库中的主副本。

### 改进

- Git 仓库和本地目录包含多个 Skill 时，均要求显式选择需要添加的目录。
- Manifest、Profile、更新检查和内容漂移检测统一以库主副本作为内容身份，同时保留平台启用位置的范围信息。
- 外部工具安装和旧版本遗留的实体目录继续参与盘点，但默认不会被自动接管或覆盖。

### 修复

- 修复带内容哈希的 Manifest 重复应用时误判为替换的问题。
- 修复通过目录连接启用的 Git Skill 无法检查更新的问题。
- 修复启用后无法识别库主副本本地内容变化的问题。
